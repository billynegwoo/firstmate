...
The effort answer is validated independently and affects dispatch only when quota selection chooses a candidate whose effort is `auto`.
For that candidate, a missing or malformed answer yields `error`, confidence below the same 0.6 floor yields `ambiguous`, and a valid answer resolves to the highest supported level at or below Jev's choice for its harness and model.
Effort classification leaves the configured model and quota ranking unchanged; fixed and omitted efforts are preserved regardless of the effort answer.
