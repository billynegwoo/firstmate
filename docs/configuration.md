...
The effort answer is validated on its own, in the rule answer's shape over exactly the four offered levels, and consulted only when the chosen candidate declared `auto`: a missing or malformed effort answer is then an `error`, an effort confidence below the same 0.6 floor is `ambiguous`, and otherwise the level is lowered to the highest one the resolver's harness-and-model effort check accepts, so the `profile:` line never carries `auto` or an unsupported level.
