...
It proves an `auto` profile adds the effort Choice to that same request with exactly `low`, `medium`, `high`, and `xhigh`, that the effort answer is validated and floored on its own, lowered to each harness's supported level and omitted on `gemini`, `opencode`, `kimi`, and `cursor`, that a pinned effort is untouched by any effort answer, and that configurations without `auto` ask and read no effort question.
...
Live effort-classification evidence is refreshed the same way with an `auto` profile in the rules file; the calibration briefs used so far stay in private task evidence rather than here.
